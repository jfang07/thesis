...
noisily display _n _dup(80) "*" _n											 ///
	"FIGURE 6. TIMING OF WELFARE REFORM EFFECTS FOR ADDITIONAL DAUGHTER "	 ///
	"OUTCOMES OF INTEREST" _n _dup(80) "*" _n
********************************************************************************
...
foreach outcomevar of varlist D_noearn D_EPOV100 D_single D_movedlastyear {
	...
	capture ivreg2 `outcomevar' (M1 ${R_M1_Y_balanced} = ${R_Y_IV_balanced}) ///
		${R_Y_balanced} ${X} ${SD} ${YD} ${MSD} [aw=D_CLWT], ///
		cluster(state) partial(${R_Y_balanced} ${X} ${SD} ${YD} ${MSD})
	...
	capture ivreg2 `outcomevar' (M1 ${R_M1_Y_balanced} = ${R_Y_IV_balanced}) ///
		${R_Y_balanced} ${X} ${SD} ${YD} ${MSD} ${statetrend} [aw=D_CLWT],	 ///
		partial(${R_Y_balanced} ${X} ${SD} ${YD} ${MSD} ${statetrend})		 ///
		cluster(state)
	...
	capture ivreg2 `outcomevar' (M1 ${R_M1_Y_balanced} = ${R_Y_IV_balanced}) ///
		${R_Y_balanced} ${X} ${SD} ${YD} ${MSD} ${statetrend2} [aw=D_CLWT],	 ///
		partial(${R_Y_balanced} ${X} ${SD} ${YD} ${MSD} ${statetrend2})		 ///
		cluster(state)
	...
	putexcel set JPE-MS-2017407_Figure_6_output, sheet("`outcomevar'") modify
