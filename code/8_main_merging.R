...
# Load  data
merged_exp18 <- read_dta("data/merged_exp18.dta")
adj_merged_hds <- read_dta("data/adj_merged_hds.dta")

# Merge with adult children, starting with moms
pairs_mom <- left_join(x = long_indiv_dropped, y = merged_exp18, 
                       by = c("id",  "id_mom" = "id_hd_exp"))

# Keep only dads 
merged_exp_dads <- merged_exp18 %>% 
  filter(hd_exp == 1) %>% 
  rename(id_dad = id_hd_exp)
...
# Keep only observations with non-missing values for average long-term wages
unranked18 <- merged_par_dropped %>% 
  filter(!is.na(avg_adj_wages_hd_exp) & !is.na(avg_adj_indiv_wages))


# Calculate percent dropped due to missing average long-term wages
(nrow(merged_par_dropped) - nrow(unranked18))/nrow(merged_par_dropped)*100
(nrow(merged_par_dropped[merged_par_dropped$sex == 1,]) - 
    nrow(unranked18[unranked18$sex == 1,]))/nrow(merged_par_dropped[merged_par_dropped$sex == 1,])*100
(nrow(merged_par_dropped[merged_par_dropped$sex == 2,]) - 
    nrow(unranked18[unranked18$sex == 2,]))/nrow(merged_par_dropped[merged_par_dropped$sex == 2,])*100

# Export data
write_dta(unranked18, "data/unranked18.dta")
